        ...
        transform = torchvision.transforms.Compose(
            [
                torchvision.transforms.Resize(
                    (args.resize, args.resize)) if args.resize is not None else torchvision.transforms.Lambda(
                    lambda x: x),
                torchvision.transforms.RandomRotation(args.randrot) if (
                            args.randrot is not None and train) else torchvision.transforms.Lambda(lambda x: x),
                torchvision.transforms.RandomHorizontalFlip(args.randhf) if (
                            args.randhf is not None and train) else torchvision.transforms.Lambda(lambda x: x),
                torchvision.transforms.RandomVerticalFlip(args.randvf) if (
                            args.randvf is not None and train) else torchvision.transforms.Lambda(lambda x: x),
                torchvision.transforms.ColorJitter(brightness=args.randjit, contrast=args.randjit,
                                                   saturation=args.randjit, hue=args.randjit) if (
                            args.randjit is not None and train) else torchvision.transforms.Lambda(lambda x: x),
                torchvision.transforms.ToTensor(),
                torchvision.transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224,
                                                                                  0.225]) if args.imnorm else torchvision.transforms.Lambda(
                    lambda x: x)
            ]
        )
        return transform
    ...
    if args.dataset in ['FEMNIST', 'Shakespeare', 'Sent140', 'CelebA',
        ...
        if args.dataset in ['FEMNIST', 'CelebA']:
            ...
            if args.resize is None:
                logger.info(
                    f'[LOAD] Dataset `{args.dataset}` may require `resize` argument; (recommended: `FEMNIST` - 28, `CelebA` - 84)!')
        ...
        split_map, client_datasets, args = fetch_gleam(args=args, root=args.data_path, seed=args.seed,
                                                       test_fraction=args.test_fraction, seq_len=args.seq_len)
